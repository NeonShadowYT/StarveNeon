...
public class BuildingSystem : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (_buildingBlockOnScene == null) CreateNewBlock();
            ...
            if (LayerMask.LayerToName(hit.collider.gameObject.layer) == _connectionType)
            {
                ...
                if (_blockConnection.parent.GetComponentInParent<HouseManager>() != null) RemoveHouseBlocksFromCollidersList(); else RemoveFirstBlockFromCollidersList();
            }
            else
            {
        ...
        else if (_buildingBlockOnScene != null) Destroy(_buildingBlockOnScene);
    }
    private void SetConnectionTypeBasedOnBuildingBlock()
    {
        if (currentBuildingBlock.GetComponent<Foundation>() != null) _connectionType = "FoundationConnection";
        else if (currentBuildingBlock.GetComponent<EdgeBlock>() != null) _connectionType = "EdgeConnection";
        else if (currentBuildingBlock.GetComponent<Ceiling>() != null) _connectionType = "CeilingConnection";
        else if (currentBuildingBlock.GetComponent<Door>() != null) _connectionType = "DoorConnection";
    }
    private void RemoveFirstBlockFromCollidersList()
    {
        if (_buildingBlockOnScene.GetComponent<BuildingBlock>().detectedColliders.Contains(_blockConnection.parent.GetComponent<Collider>())) _buildingBlockOnScene.GetComponent<BuildingBlock>().detectedColliders.Remove(_blockConnection.parent.GetComponent<Collider>());
    }
    private void RemoveHouseBlocksFromCollidersList()
    {
        foreach (BuildingBlock buildingBlock in _blockConnection.parent.GetComponentInParent<HouseManager>().buildingBlocks) if (_buildingBlockOnScene.GetComponent<BuildingBlock>().detectedColliders.Contains(buildingBlock.GetComponent<Collider>())) _buildingBlockOnScene.GetComponent<BuildingBlock>().detectedColliders.Remove(buildingBlock.GetComponent<Collider>());
    }
    ...
    private void CreateNewBlock()
    {
        ...
        if(_buildingBlockOnScene.GetComponent<Collider>() != null) _buildingBlockOnScene.GetComponent<Collider>().enabled = false;
    }
    public void PlaceBlock()
    {
        _buildingBlockOnScene.GetComponent<BuildingBlock>().isPlaced = true;
        if(_buildingBlockOnScene.GetComponent<Collider>() != null) _buildingBlockOnScene.GetComponent<Collider>().enabled = true;   
        if(_buildingBlockOnScene.GetComponent<IHaveConnections>() != null) _buildingBlockOnScene.GetComponent<IHaveConnections>().TurnOnConnections();      
    ...
    private void OnEnable() => ChangeBuildingBlock();
    private void OnDisable() => Destroy(_buildingBlockOnScene);
}