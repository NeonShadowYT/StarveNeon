...
public class Foundation : BuildingBlock, ICheckColliders, IHaveConnections
{
    ...
    {
        for (int i = 0; i < foundationConnections.Count; i++) foundationConnections[i].SetActive(true);
    ...
    public void CheckColliders()
    {
        ...
        Collider[] currentColliders = Physics.OverlapBox(transform.position, new Vector3(3f, 0.5f, 3f), transform.rotation, ~LayerMask.GetMask("Terrain", "FoundationConnection", "EdgeConnection", "CeilingConnection", "DoorConnection"));
        detectedColliders.AddRange(currentColliders);
    }
    public void CheckNearbyFoundations(int i)
    {
        Collider[] overlapColliders = Physics.OverlapSphere(foundationConnections[i].transform.position, foundationConnections[i].GetComponent<SphereCollider>().radius);
    ...
    {
        Collider[] overlapColliders = Physics.OverlapSphere(edgeConnections[i].transform.position + Vector3.down * 1.5f, 0.5f, LayerMask.GetMask("EdgeConnection"));
