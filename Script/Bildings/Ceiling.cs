...
using UnityEngine;
public class Ceiling : BuildingBlock, ICheckColliders, IHaveConnections
{
    ...
    public void CheckColliders()
    {
        ...
        Collider[] currentColliders = Physics.OverlapBox(transform.position, _colliderExtents, transform.rotation, ~LayerMask.GetMask("FoundationConnection", "EdgeConnection", "CeilingConnection", "DoorConnection"));
        detectedColliders.AddRange(currentColliders);
    }
    public void CheckNearbyCeilings(int i)
    {
        Collider[] overlapColliders = Physics.OverlapSphere(ceilingConnections[i].transform.position, ceilingConnections[i].GetComponent<SphereCollider>().radius);
    ...
    {
        foreach (var ceilingConnection in ceilingConnections) ceilingConnection.SetActive(true);
    }
}
