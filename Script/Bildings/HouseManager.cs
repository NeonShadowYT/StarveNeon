    ...
    public void UpdateBuildingBlocks()
    {
        ...
        foreach (BuildingBlock buildingBlock in buildingBlocks)
        {
            if (buildingBlock.GetComponent<Foundation>() != null)
            {
                ...
                for (int i = 0; i < foundation.foundationConnections.Count; i++) foundation.CheckNearbyFoundations(i);
                for (int i = 0; i < foundation.edgeConnections.Count; i++) foundation.CheckNearbyEdgeConnections(i);
            }
            if (buildingBlock.GetComponent<EdgeBlock>() != null)
            {
                EdgeBlock edgeBlock = buildingBlock.GetComponent<EdgeBlock>();
                for (int i = 0; i < edgeBlock.ceilingConnections.Count; i++) edgeBlock.CheckNearbyCeilings(i);
            }
            if (buildingBlock.GetComponent<Ceiling>() != null)
            {
                Ceiling ceiling = buildingBlock.GetComponent<Ceiling>();
                for (int i = 0; i < ceiling.ceilingConnections.Count; i++) ceiling.CheckNearbyCeilings(i);
            }
        } 
    }
}
