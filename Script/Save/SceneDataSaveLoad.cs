...
using UnityEngine;
public class SceneDataSaveLoad : MonoBehaviour //https://www.youtube.com/channel/UC2pH2qNfh2sEAeYEGs1k_Lg
{
    [SerializeField] private Transform _savingEnvironment;
    public void SaveScene() => BinarySavingSystem.SaveScene(_savingEnvironment);
    public void LoadScene()
    {
        for (int i = 0; i < _savingEnvironment.childCount; i++) Destroy(_savingEnvironment.GetChild(i).gameObject);
    ...
    private static string GetPrefabName(SceneData data, int i)
    {
        ...
        else prefabName = data.objectNames[i];
        return prefabName;
    }
}
