...
public class BuildingBlock : MonoBehaviour
{
    ...
    {
        if(GetComponent<BoxCollider>() != null) _colliderExtents = GetComponent<BoxCollider>().size;
    ...
    private void UpdateMaterials()
    {
        foreach (MeshRenderer meshRenderer in _meshRenderers)
        {
            ...
            for (int i = 0; i < redMaterials.Length; i++) redMaterials[i] = _redMaterial;
            Material[] greenMaterials = new Material[meshRenderer.materials.Length];
            for (int i = 0; i < greenMaterials.Length; i++) greenMaterials[i] = _greenMaterial;
            meshRenderer.materials = detectedColliders.Count > 0 ? redMaterials : greenMaterials;
        }
    }
    public void ChangeToBlockMaterial()
    {
        foreach (MeshRenderer meshRenderer in _meshRenderers) meshRenderer.materials = _blockMaterials;
    }
}